...
% TEST TO LEARN ABOUT GITHUB AND ATOM
%   Synopsis:
...
%-------------------------------------------------------------------------
%initialize all empty cell arrays
TERM_STRUCTURE_FORMAT = [];
...
VXV_HistoricalSave = websave('vxvcurrent.csv', 'http://www.cboe.com/publish/scheduledtask/mktdata/datahouse/vxvdailyprices.csv');
...
%XIV_Historical = readtable('XIV_data.csv');
%XIV_date = table2cell(XIV_Historical(:,1));
%XIV_price = table2cell(XIV_Historical(:,5));
%XIV_cellarray(:,1) = XIV_date;
%XIV_cellarray(:,2) = XIV_price;
%XIV_date = datetime(XIV_cellarray(:,1), 'Format', 'mm/dd/yyyy');
%%%XIV_date = datetime(XIV_cellarray(:,1), 'Format', 'MM/dd/yyyy');
%XIV_cellarray(:,1) = cellstr(XIV_date);

%catch
%    msg = 'Google is being a dick and not letting you download historical data';
%    uiwait(msgbox('Google is being a dick and not letting you download historical data'));
%    error(msg);
%end

%    NOTE: we could speed this up by setting the loop
%           counter equal to the previous "hit" so it
%           doesnt need to start from the beginning again
%
%   1.6) Format VIX Paramters
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
html = urlread(historicalCBOEwebaddress);
...
txt = regexprep(txt,'<.*?>','');
online_int1 = strsplit(txt, '\t');
online_int2 = online_int1{1,17};
CONTRACT_NAMES_CBOE_WEBPAGE_ACT = strsplit(online_int2, '  ');
    ...
    contract_name1 = CONTRACT_NAMES_CBOE_WEBPAGE_EXP{s4a};
        ...
        for s4b = 1:numel(CONTRACT_NAMES_CBOE_WEBPAGE_ACT)
            ...
            if  isequal(contract_name1, contract_name2)
                ...
                %format contract download string for csv file names using string split
                Contract_Names_Online_Temp2 = strsplit(Contract_Names_Online_Temp1{s4a}, {' ',')'});
                Contract_Names_Online_Temp3 = cat(2,'CFE_',Contract_Names_Online_Temp2{1,2},Contract_Names_Online_Temp2{1,4},'_VX.csv');
...
% Dick fuckin move by CBOE here... %change the format in june 2013....
% assholes did it again by adding then removing the final trading date in
% november of 2014.
for s5a = 1:length(Contract_Name_Array)

     if isequal(Contract_Name_Array{s5a}, 'CFE_M13_VX.csv')
         %CBOE adjustment #1 stop row
         stop_row = s5a;
     end

    % dont delete last row for contracts before 'CFE_X14_VX' nov 2104
     if isequal(Contract_Name_Array{s5a}, 'CFE_X14_VX.csv')
         %CBOE adjustment #2 stop row
         stop_row2 = s5a;
         %CBOE adjustment #3 stop row
         stop_row3 = s5a - 1;
     end
...
%last contract name
LastContract = ContractNamesfromStructure{end};
%check last contract size
[nr_LastContract, nc_LastContract] = size(ContractsAsStructure.(LastContract));

% how many entries in contract names cell array?
% ISSUE relating to whether the most recent contract added to CBOE has only
% one row or not. if it does, then disregard it. The problem is that we
% delete the last row previously in the code. we should figure out how to
...
for s5c = 1:NumOfTotalContracts
    ...
    % delete expiration date row from each contract.
    %--------------------------------------------------   XXXXXXXXXXXXX
...
for s6a = 1:length(AllContracts_vector)
    ...
    length_each_contract = cellfun('length',AllContracts_vector);
    ...
    for s6b = 1:total_testrows
        ...
        %clear the contents of the copied cells (Date and Price)
        COMPLETE_OUTPUT_ARRAY{s6b,s6_c2 + 1} = [];
        COMPLETE_OUTPUT_ARRAY{s6b,s6_c2} = [];

        for s6c = 1:Total_Vix_Rows
            ...
            TestDate_VIX = COMPLETE_OUTPUT_ARRAY(s6c,1);
                ...
                if isequal(TestDate_Contracts, TestDate_VIX)
                    ...
                    COMPLETE_OUTPUT_ARRAY{s6c,s6_c1 + 1} = TestDate_VIX;
                    COMPLETE_OUTPUT_ARRAY{s6c,s6_c1 + 2} = TestPrice_Contracts;
...
%remove all empty cells from cell array
emptycells7 = cellfun('isempty', COMPLETE_OUTPUT_ARRAY);
%calculate size of output array
[nr_Complete, nc_Complete] = size(COMPLETE_OUTPUT_ARRAY);
%initialize counter
s7_r = 1;
%initialize cell array
CBOE_TERM_STRUCTURE = cell(COMPLETE_OUTPUT_ARRAY);
CBOE_TERM_STRUCTURE(:) = [];
...
for s7a = 1:nr_Complete
        ...
        Data = COMPLETE_OUTPUT_ARRAY{s7a,s7b};
...
clear('output_file_AllParameters','output_file_SmithParameters','ans','AllLabels','SmithLabels','Tdate','TdateNum','TdateStr','TDiff','TDiffCo','TDiffnumer','y','ContractExpirationDates','VXMT_date','VXST_date','VXV_date','XIV_date','V_array','V_VIX','V_VXV','V_VXST','V_VXMT','V_XIV','s1a','s1b','s2_r','s4a','s4b','s5_c1','s5_c2','s5a','s5b','s5c','s5d','s5e','s5f','s6_c1','s6_c2','s6a','s6b','s6c','s7_c','s7_r','s7a','s7b','Today','ALL_CONTRACTS_WEBSAVE','CBOE_TERM_STRUCTURE','CBOE_TERM_STRUCTURE_FINAL','CBOE_TERM_STRUCTURE_FINAL_TABLE','CBOE_TERM_STRUCTURE_TABLE','CONTRACT_NAMES_CBOE_WEBPAGE_ACT','CONTRACT_NAMES_CBOE_WEBPAGE_EXP','CONTRACT_NAME_ARRAY_update','ContractDateColumn','ContractNamesfromStructure','ContractPriceColumn','Contract_Names_Online_Temp1','Contract_Names_Online_Temp2','Contract_Names_Online_Temp3','Contract_Names_String_Webpage','Data','FILE_SAVE','FutureContracts_RowsAsTradeDates','FutureContracts_RowsNumbered','Future_Contracts','IndividualContract_cellarray','IndividualContract_table','NumOfTotalContracts','TERM_STRUCTURE_FORMAT','TestDate_Contracts','TestDate_VIX','TestPrice_Contracts','Total_Contract_Columns','VIX_CellArray','VIX_HistoricalSave','VIX_Tablefile','VIX_vector','VXMT_Historical','VXMT_HistoricalSave','VXMT_cellarray','VXMT_vector','VXST_Historical','VXST_HistoricalSave','VXST_cellarray','VXST_vector','VXV_Historical','VXV_HistoricalSave','VXV_cellarray','VXV_vector','XIV_cellarray', 'XIV_ClosePrice', 'XIV_ClosePriceNEST', 'XIV_final', 'XIVfinal', 'XIV_temp', 'XIV_TradeDate', 'XIV_TradeDateNEST', 'LastContract', 'nc_LastContract','nr_LastContract','ans','contract_name1','contract_name2','emptycells1','emptycells2','emptycells3','emptycells4','emptycells5','emptycells6','emptycells7','emptycells8','emptycells9','emptycells10','emptycells11','emptycells12','emptycells13','emptycells14','emptycells15','emptycells16', 'historicalCBOEwebaddress','html','length_each_contract','month_converted','month_name','month_total','nc_Complete','nc_ContractDates','nc_ContractUnderTest','nc_VIX','nr_Complete','nr_ContractDates','nr_ContractUnderTest','nr_VIX','online_int1','online_int2','options','stop_row','stop_row2','stop_row3','total_testrows','txt','webaddress','year_converted','year_total');
...
clear('TradeDate','Total_Vix_Rows','V_PARAMETERS','VIX','VX1','VX2','VX3','VX4','VX5','VX6','VX7','VX8','VX9','VXMT','VXST','VXV','XIV','XIV_TableData');
%run the parameters
